...
// LazySwapPassABIJSON is the minimal LazySwapPass (ERC-721) ABI: the payable
// mint plus the view calls needed to show a wallet's pass validity + expiry.
const LazySwapPassABIJSON = `[
  {"name":"mint","type":"function","inputs":[],"outputs":[],"stateMutability":"payable"},
  {"name":"hasValidPass","type":"function","inputs":[{"name":"user","type":"address"}],"outputs":[{"type":"bool"}],"stateMutability":"view"},
  ...
  {"name":"expiresAt","type":"function","inputs":[{"name":"tokenId","type":"uint256"}],"outputs":[{"type":"uint256"}],"stateMutability":"view"}
]`
...
// LazySwapPassABI is the parsed LazySwapPass ABI, initialised at package load.
var LazySwapPassABI abi.ABI
...
func init() {
	...
	a, err = abi.JSON(strings.NewReader(LazySwapPassABIJSON))
	if err != nil {
		panic("parse LazySwapPass ABI: " + err.Error())
	}
	LazySwapPassABI = a
}
